...
export function GaleriaFotos({ imagens, nome }: { imagens: Imagem[]; nome: string }) {
  ...
  if (imagens.length === 0) {
    return (
      <div className="flex aspect-square w-full items-center justify-center rounded-2xl border border-border bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f]">
        <Icon name="motorcycle" className="h-20 w-20 text-muted" />
      </div>
    );
  }
